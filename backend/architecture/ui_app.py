...
async def chat_interface(user_message, history, state):
    ...
    if new_state.get("awaiting_action") == "executing":
        yield "", history, new_state, "**Statut :** ⏳ L'Executor orchestre l'exécution du plan en ce moment..."
        
        try:
            # Transfer the plan to the executor
            plan_str = json.dumps(new_state.get("plan", {}), ensure_ascii=False)
            executor_response = await _call_acp_agent("executor", plan_str)
            
            # Formate response nicely
            try:
                res_json = json.loads(executor_response)
                final_text = res_json.get("final_result", executor_response)
                if isinstance(final_text, str):
                    try:
                        final_text_json = json.loads(final_text)
                        formatted_response = json.dumps(final_text_json, ensure_ascii=False, indent=2)
                    except:
                        formatted_response = final_text
                else:
                    formatted_response = json.dumps(final_text, ensure_ascii=False, indent=2)
            except json.JSONDecodeError:
                formatted_response = executor_response
                
            history.append(("Exécution du plan", f"**✅ Résultat final de l'Executor :**\n```json\n{formatted_response}\n```"))
            new_state["awaiting_action"] = None
            yield "", history, new_state, "**Statut :** ✅ Exécution terminée."
        except Exception as e:
            history.append(("Exécution du plan", f"❌ **Erreur de l'Executor :** {str(e)}"))
            new_state["awaiting_action"] = None
            yield "", history, new_state, "**Statut :** ❌ Erreur lors de l'exécution."
    else:
        yield "", history, new_state, "**Statut :** 🟢 En attente d'une nouvelle action"
...
def get_latest_metrics(agent_name: str) -> str:
    ...
    try:
        with open(latest_file, "r", encoding="utf-8") as f:
            ...
            elif agent_name == "intelligent_target_discovery":
                markdown += "#### Statistiques API Discovery\n"
                markdown += f"- **Requêtes API envoyées:** {data.get('api_requests_sent', 0)}\n"
                markdown += f"- **Erreurs API:** {data.get('api_errors', 0)}\n"
...
def refresh_metrics():
    ...
    return get_latest_metrics("planner"), get_latest_metrics("executor"), get_latest_metrics("intelligent_target_discovery")
...
with gr.Blocks(title="Système Multi-Agent PI - Dashboard", theme=gr.themes.Soft()) as demo:
    ...
    with gr.Tabs():
        ...
        with gr.Tab("Interaction Planner"):
            ...
            tracking_status = gr.Markdown("**Statut :** 🟢 En attente d'une nouvelle action")
            chatbot = gr.Chatbot(height=500, label="Dialogue avec le Système")
            ...
            msg.submit(chat_interface, [msg, chatbot, state], [msg, chatbot, state, tracking_status])
            
            def clear_chat():
                return [], {"awaiting_action": None, "analysis": {}, "plan": {}}, "**Statut :** 🟢 En attente d'une nouvelle action"
                
            clear.click(clear_chat, inputs=None, outputs=[chatbot, state, tracking_status])
        ...
        with gr.Tab("Dashboard Métriques (Performances)"):
            ...
            with gr.Row():
                ...
                with gr.Column():
                    ...
                    discovery_metrics_display = gr.Markdown("*(Cliquez sur Rafraîchir)*")
                    
            refresh_btn.click(refresh_metrics, inputs=None, outputs=[planner_metrics_display, executor_metrics_display, discovery_metrics_display])
            demo.load(refresh_metrics, inputs=None, outputs=[planner_metrics_display, executor_metrics_display, discovery_metrics_display])
