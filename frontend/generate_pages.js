...
const agents = [
  { id: 'agent', title: 'Target Discovery', desc: 'Automated target extraction and molecule generation', comp: 'DiscoveryAgentView' },
  { id: 'safety', title: 'MediSafe AI', desc: 'Predictive toxicology and clinical safety screening', comp: 'SafetyAgentView' },
  { id: 'hypothesis-generator', title: 'Hypothesis Generator', desc: 'AI-driven research hypothesis generation based on multi-omic data patterns', comp: 'HypothesisAgentView' },
  { id: 'orchestrator', title: 'Lab Orchestrator', desc: 'Combined Planner & Executor layer managing multi-agent workflows', comp: 'OrchestratorView' },
  { id: 'reporter', title: 'Reporter Agent', desc: 'Automated justification and technical report generation', comp: 'ReporterAgentView' },
  { id: 'data-manager', title: 'Data Manager', desc: 'Knowledge indexing and vector storage for research data', comp: 'DataManagerView' },
  { id: 'automation', title: 'Lab Automation', desc: 'Hardware orchestration for 3D bioprinting and liquid handling', comp: 'AutomationView' }
];

const template = (title, desc, comp) => `"use client";

import ${comp} from "@/components/Dashboard/${comp}";
...
agents.forEach(agent => {
  const dirPath = path.join('./src/app', agent.id);
  if (!fs.existsSync(dirPath)) {
    fs.mkdirSync(dirPath, { recursive: true });
  }
  const pagePath = path.join(dirPath, 'page.tsx');
  if (fs.existsSync(pagePath)) {
    fs.renameSync(pagePath, path.join(dirPath, 'page.tsx.bak'));
  }
  fs.writeFileSync(pagePath, template(agent.title, agent.desc, agent.comp));
});
