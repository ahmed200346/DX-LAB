...
import { Cpu, Printer, Play, Square, Settings, Activity, Gauge, Terminal, AlertCircle, Loader2, Bot } from "lucide-react";
...
export default function AutomationView() {
  ...
  const toggleQueue = () => {
    ...
    if (!isRunning) {
        setPrintProgress(0);
        setLogs([
            { time: new Date().toLocaleTimeString([], { hour12: false }), msg: "Initializing OT-2 workspace..." },
            { time: new Date().toLocaleTimeString([], { hour12: false }), msg: `Loading protocol: ${protocol}` },
        ]);
    }
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-500 dark:text-white/40 text-sm">Orchestration and monitoring of robotics and fluidics</p>
        </div>
        ...
        <CardContent className="p-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
            <div className="md:col-span-3 space-y-2">
              <label className="text-xs font-semibold text-gray-600 dark:text-gray-500 dark:text-white/40 uppercase tracking-wider">Select Automation Protocol</label>
              <Select value={protocol} onValueChange={setProtocol}>
                <SelectTrigger className="bg-white dark:bg-white/5 border-gray-200 dark:border-white/10 h-11">
                  <SelectValue placeholder="Choose a protocol to execute..." />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="CellTiter-Glo_Apoptosis_v3.py">CellTiter-Glo Apoptosis Assay (Venetoclax)</SelectItem>
                  <SelectItem value="Library_Preparation_NGS.py">NGS Library Preparation</SelectItem>
                  <SelectItem value="Serial_Dilution_pKd.py">Serial Dilution for Binding Affinity</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div className="md:col-span-1">
              <Button 
                ...
                disabled={!protocol}
                className={cn(
                  "w-full h-11 font-bold shadow-lg transition-all rounded-lg",
      ...
      {!isRunning && printProgress === 0 ? (
        <div className="flex flex-col items-center justify-center h-64 rounded-2xl border-2 border-dashed border-gray-200 dark:border-white/10 text-center gap-4 animate-in fade-in duration-500">
          <Bot className="w-12 h-12 text-gray-300 dark:text-white/10" />
          <div>
            <p className="text-sm font-semibold text-gray-500 dark:text-white/30">Robotics Standby</p>
            <p className="text-xs text-gray-400 dark:text-white/20 mt-1">Select and start a protocol to monitor live execution</p>
          </div>
        </div>
      ) : (
                ...
                <AnimatePresence>
                  {isRunning && (
                    ...
                    >
                       ...
                       <span className="text-[10px] font-bold text-emerald-700 dark:text-emerald-400 uppercase tracking-widest">Live Execution</span>
                    </motion.div>
                  )}
                </AnimatePresence>
                <CardHeader>
                   <div className="flex justify-between items-start">
                      <div className="flex items-center gap-3">
                         ...
                         <div>
                            ...
                            <CardDescription className="text-gray-600 dark:text-gray-500 dark:text-white/40">{isRunning ? `Executing: ${protocol}` : "Queue Paused"}</CardDescription>
                         </div>
                      </div>
                ...
                <CardContent className="space-y-6">
                   <div className="space-y-2">
                      <div className="flex justify-between text-xs">
                         <span className="text-gray-600 dark:text-gray-500 dark:text-white/40 font-bold uppercase tracking-widest">Protocol Progress</span>
                         <span className="text-emerald-700 dark:text-emerald-400 font-bold">{printProgress}%</span>
                      </div>
                      <Progress value={printProgress} indicatorClassName="bg-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.3)]" />
                   </div>
