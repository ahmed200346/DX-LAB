...
import { 
  Play, RotateCcw, Download, ExternalLink, Activity, Database, FlaskConical,
  CheckCircle2, Circle, Loader2, Info, Beaker, AlertCircle
} from "lucide-react";
  ...
  useEffect(() => {
    ...
    const initViewer = async () => {
      try {
        ...
        viewer = w.$3Dmol.createViewer(el, { backgroundColor: '#020617' });
    ...
    return () => { clearTimeout(timer); if (viewer) viewer.clear(); };
  }, [smiles, viewerId]);
...
export default function DiscoveryAgentView() {
  ...
  const handleStart = async () => {
    ...
    await startPipeline(protein, disease);
  };
                   ...
                   <CardContent>
                     <div className="space-y-4">
                       <div className="flex items-end justify-between">
                         <p className="text-4xl font-bold text-gray-900 dark:text-white tracking-tighter">{protein.toUpperCase()}</p>
                       ...
                       <div className="grid grid-cols-2 gap-4 pt-4 border-t border-gray-200 dark:border-white/5">
                         <div>
                           <p className="text-xs text-gray-600 dark:text-gray-500 dark:text-white/40">Disease Context</p>
                           <p className="text-xl font-bold text-blue-700 dark:text-blue-400">{disease}</p>
                         </div>
                         <div>
                           <p className="text-xs text-gray-600 dark:text-gray-500 dark:text-white/40">Run ID</p>
                           <p className="text-xl font-bold font-mono text-xs">{runId || "N/A"}</p>
                         </div>
                       </div>
                     </div>
                   </CardContent>
               ...
               {steps[2]?.status === "completed" && (
                 <Card className="bg-gradient-to-br from-purple-600/10 to-pink-600/5 border-gray-200 dark:border-white/10 relative overflow-hidden group animate-in zoom-in-95 duration-500">
                   ...
                   <CardContent>
                     <div className="space-y-4">
                       <div className="flex items-end justify-between">
                         <p className="text-4xl font-bold text-gray-900 dark:text-white tracking-tighter">{candidates.length}<span className="text-xl text-gray-600 dark:text-gray-500 dark:text-white/40"> candidates</span></p>
                         <Badge className="bg-purple-500/20 text-purple-700 dark:text-purple-400 border-purple-300 dark:border-purple-500/30">Ready</Badge>
                       </div>
                       <div className="pt-4 border-t border-gray-200 dark:border-white/5">
                         <div className="flex justify-between text-xs mb-1.5">
                           <span className="text-gray-600 dark:text-gray-500 dark:text-white/40">Pipeline Status</span>
                           <span className="text-gray-900 dark:text-white/90">{steps.every(s => s.status === "completed") ? "Complete" : "In Progress"}</span>
                         </div>
               ...
               <div className="space-y-6 animate-in fade-in duration-700">
                 ...
                 <div className="space-y-6">
                   {candidates.map((mol, idx) => {
                     ...
                     const boltzDone = steps[4]?.status === "completed";

                     if (typeof window !== 'undefined' && boltzDone) {
                       setTimeout(async () => {
                         try {
                           const w = window as any;
                           if (w.$3Dmol) {
                             ...
                             <div className="rounded-xl border border-gray-200 dark:border-white/10 bg-[#0a0a0f] flex flex-col overflow-hidden h-full relative">
                               <div className="bg-black/60 border-b border-white/5 px-4 py-2 text-[10px] font-bold text-gray-400 uppercase tracking-widest flex justify-between items-center z-10 absolute top-0 w-full backdrop-blur-sm">
                                 <span className="flex items-center gap-2">
                                   {!boltzDone && <Loader2 className="w-3 h-3 animate-spin text-blue-500" />} 
                                   3D Structure (Boltz-1)
                                 </span>
                                 {boltzDone && <Badge variant="outline" className="text-[9px] h-4 border-blue-500/30 text-blue-400">Interactive</Badge>}
                               </div>
